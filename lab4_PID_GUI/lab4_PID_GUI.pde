...
public void ResetIntegrator(int theValue) {
  ...
  cp5.getController("P").setValue(0);
  cp5.getController("I").setValue(0);
...
int state = 0;
public void drawSquare(float w, float h, float move_speed)
{
if (state == 0) {
    xr = xr + move_speed;
...
public void drawCircle(float radius, float speed)
{
    xr = cos(angle) * radius;
    yr = sin(angle) * radius;
    angle = angle + speed;
...
while(1==1) {
    ...
    if(haplyBoard.data_available()){
      ...
      //drawSquare(0.35, 0.35, 0.0001); // technically allows for a rectangle
      drawCircle(0.3, 0.0003);
      println(xr + ", " + yr);
